<script lang="ts">
  // Test-only host: sets the form + config contexts Submit needs, since the
  ...
  interface Props extends ComponentProps<typeof Submit> {
    form: ValidatableForm;
    uiConfig?: PartialUiConfig;
    extraRegistrations?: FieldRegistration[];
  }

  const { form, uiConfig, extraRegistrations, ...submitProps }: Props =
    $props();
